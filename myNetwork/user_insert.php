...
$run_email=mysqli_query($con,$get_email)or die('Could not look up user information; ' . mysqli_error($con));
...
if(strlen($pass)<8){
 echo "<script>alert('Password has to be more than 8 characters');</script>";
 exit();
}
else{
...
$run_insert =mysqli_query($con,$insert);
  ...
  if($run_insert){
    ...
    die('Could not look up user information; ' . mysqli_error($con));
  }
}
}
